...
//Main code
void RaveTest(const char *inputFile, double dRjetsMax, int label, int max_ev, const char *result) {
    ...
    //factory.setDefaultMethod("avr-minweight:0.5-primcut:2-seccut:6"); // Supposed default settings
    ...
    for (entry = 1; (entry < allEntries)&&(entry < max_ev); ++entry) {
        ...
        if (JetJ[1])
            myfile << "    Jet 2    pT: " << PTJ[1] << " eta: " << EtaJ[1] << " phi: " << PhiJ[1]
                   << endl;
        if (JetJ[0])
            myfile << "n_vert jet_association sum_sqr_PT vert_mult vert_ D0 vert_Eta vert_Phi deltaR1 deltaR2 vert_chisq" << endl;
        // Vertexing
