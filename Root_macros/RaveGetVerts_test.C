...
//Main code
void RaveGetVerts_test(const char *inputFile, double dRjetsMax, int label, int max_ev, const char *result) {
        ...
        for (vector<rave::Vertex>::const_iterator r = event_vertices.begin(); r != event_vertices.end(); ++r) {
            ...
            for (vector<rave::Track>::const_iterator t = tracks.begin(); t != tracks.end(); ++t) {
                ...
                if (deltaR1 < dRjetsMax) {
                    vert_mult_j1 ++;
                    vert_Px_j1 += t->momentum().x();
                    vert_Py_j1 += t->momentum().y();
                }
                if (deltaR2 < dRjetsMax) {
                    vert_mult_j2 ++;
                    vert_Px_j2 += t->momentum().x();
                    vert_Py_j2 += t->momentum().y();
                }
            }
