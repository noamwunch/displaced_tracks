...
//Main code
void RaveGetVertsByJet(const char *inputFile, double dRjetsMax, int label, int max_ev, const char *result) {
    ...
    for (entry = 1; (entry < allEntries)&&(entry < max_ev); ++entry) {
        ...
        for (vector<rave::Vertex>::const_iterator r = event_vertices.begin(); r != event_vertices.end(); ++r) {
            ...
            if (deltaR2 < dRjetsMax)
                myfile << n_vert << " " << 2 << " " << vert_disp << " " << vert_mult << " " << vert_PT << endl;
            n_vert = n_vert + 1;
        }
    }
    myfile << "Done" << endl;
