...
void Example5D()
{
  ...
  for(entry = 1; entry < allEntries; ++entry)
  {
   ...
   //MET
    ...
    if(p1Ass && p2Ass)
    {
      ...
      for(i = 0; i < branchTrack->GetEntriesFast(); ++i)
      {
        ...
        if(deltaR1 < dRjetsMax)
          myfile << entry << " " << "1 1 " << track->PT << " " << track->EtaOuter << " " << track->PhiOuter << " " << deltaR1 << " " << track->PID << " " << track->D0 << " " << track->DZ << " " << track->ErrorD0 <<  " " << track->ErrorDZ <<  " " << track->Xd << " " << track->Yd << endl;

        if(deltaR2 < dRjetsMax)
          myfile << entry << " " << "2 1 " << track->PT << " " << track->EtaOuter << " " << track->PhiOuter << " " << deltaR2 << " " << track->PID << " " << track->D0 << " " << track->DZ << " " << track->ErrorD0 << " " << track->ErrorDZ <<  " " << track->Xd << " " << track->Yd << endl;
      }
	...
	//cout << factory.method();
	double xp = 0; double yp = 0; double zp = 0; double chisq=0;
	...
	for ( vector < rave::Vertex >::const_iterator r=jet1_vertices.begin(); r!=jet1_vertices.end() ; ++r )
		...
		for ( vector < rave::Track >::const_iterator t=tracks.begin(); t!=tracks.end() ; ++t )
		{
		  ...
		  cout << track5[1];	
 			   ...
 			   0 << " " << 0 << " " << track5[3] << " " << track5[4] << " " << 0 << " " << 0 << " " << 0 << " " << 0 << " " << 0 << endl;
		}  	
	}

        xp = 0; yp = 0; zp = 0; chisq=0;
	...
	for ( vector < rave::Vertex >::const_iterator r=jet2_vertices.begin(); r!=jet2_vertices.end() ; ++r )
		...
		for ( vector < rave::Track >::const_iterator t=tracks.begin(); t!=tracks.end() ; ++t )
		{
      		  myfile << entry << " " << "2 1 " << (*t).position().x() << " " << (*t).position().y() << " " << (*t).position().z() << " " <<
